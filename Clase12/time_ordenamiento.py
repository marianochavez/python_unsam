...
def ord_seleccion(lista_gen):
    """Ordena una lista de elementos según el método de selección.
       Pre: los elementos de la lista deben ser comparables.
       Post: la lista está ordenada."""
    lista = lista_gen.copy()
    comp_seleccion = 0
...
def experimento(N, k):
    ...
    for i in range(k):
        ...
        res_seleccion += ord_seleccion(lista)
        res_insercion += ord_insercion(lista)
        res_burbujeo += ord_burbujeo(lista)
        res_merge_sort += merge_sort(lista)[1]
...
def experimento_timeit(Nmax):
    ...
    comparaciones_merge_sort = []
    for N in range(1, Nmax + 1):
        lista_gen = generar_lista(N)
        comparaciones_seleccion.append(t.timeit(lambda: ord_seleccion(lista_gen.copy()), number=1))
        comparaciones_insercion.append(t.timeit(lambda: ord_insercion(lista_gen.copy()), number=1))
        comparaciones_burbujeo.append(t.timeit(lambda: ord_burbujeo(lista_gen.copy()), number=1))
        comparaciones_merge_sort.append(t.timeit(lambda: merge_sort(lista_gen.copy()), number=1))
    return comparaciones_seleccion, comparaciones_insercion, comparaciones_burbujeo, comparaciones_merge_sort

n = 100
experimento_timeit = experimento_timeit(n)
plt.plot(experimento_timeit[0], label="Selección", color = "red")
plt.plot(experimento_timeit[1], label="Inserción", color = "orange")
plt.plot(experimento_timeit[2], label="Burbujeo", color = "black")
plt.plot(experimento_timeit[3], label="Merge_Sort", color = "green")
