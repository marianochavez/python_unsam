...
def leer_camion(nombre_archivo):
    camion = fileparse.parse_csv(open(nombre_archivo), has_headers=True)
    return camion


def leer_precios(nombre_archivo):
    precios = fileparse.parse_csv(open(nombre_archivo), has_headers=False)
    return precios
    ...
    for item_camion in camion:
        for item_precio in precios:
            if item_camion['nombre'] == item_precio[0]:
                ...
                cambio = (float(item_precio[1]) - float(item_camion['precio']))
